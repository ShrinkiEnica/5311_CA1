...
TRAIN_RATIO = 0.7
SEQ_LEN = 128           # training sequence length (6.4s @ 20Hz)
BATCH_SIZE = 1024
EPOCHS = 400
LR = 3e-4
WEIGHT_DECAY = 1e-5
D_MODEL = 256           # ssm model dimension
HIDDEN_DIM = 512        # decoder hidden dimension
SSM_LAYERS = 5          # stacked SSM layers
SSM_STATE_DIM = 256     # internal recurrent state dimension
DROPOUT = 0.1
    ...
    for epoch in range(1, config["epochs"] + 1):
        ...
        if d_err < best_dream_err:
            ...
            if model_dir is not None:
                torch.save(best_state, model_dir / "wam_v3_best_dream.pt")
        ...
        if model_dir is not None and epoch % SAVE_EVERY == 0:
            torch.save({
                ...
                "history": history,
            }, model_dir / f"wam_v3_ckpt_ep{epoch}.pt")
...
def main():
    ...
    # Save model
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    model_path = MODEL_DIR / "wam_v3_best.pt"
    torch.save({
        "model_state_dict": model.state_dict(),
    ...
    VIS_DIR = Path("visualization/wam_v3")
